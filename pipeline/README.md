...
Пост в #meta-radar — оттуда же, но никогда сам собой:

```bash
python pipeline/findings.py --db data-slice.db --preview    # показать, что уйдёт
python pipeline/findings.py --db data-slice.db --post        --webhook "$META_RADAR_WEBHOOK"                      # отправить
```

Искалка работает на локальной машине, а бот живёт на сервере — достучаться до
канала иначе нечем, поэтому пост уходит веб-хуком. На экране находки по-русски,
в канал уходит английский: остальные сообщения бота там английские.
