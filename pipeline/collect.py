...
Регионы собираются ПАРАЛЛЕЛЬНО (поток на регион), внутри региона бакеты идут
сверху вниз (master → emerald → gold → silver). Riot считает лимит на пару
(ключ, хост роутинга), поэтому euw1/na1/kr жмутся одновременно и дают кратный
прирост; регионы с общим хостом MATCH-V5 (euw1+eun1 → europe) сами встают в
очередь в лимитере этого хоста и лимит не нарушают.

Когда бакет вычерпан (игроки кончились или просела доля новых матчей) — регион
переключается на следующий бакет. Ctrl+C — мягкая остановка с сохранением и
подсказкой команды публикации базы на сервер.
...
# Одно соединение на все потоки + лок на доступ. Запись занимает микросекунды
# против 1.25 с ожидания API, так что конкуренции за лок практически нет, а
# SQLite не любит одновременные транзакции из разных потоков.
# RLock (а не Lock): mark_processed вызывается уже изнутри залоченного
# process_match — с обычным локом это был бы дедлок.
...
class DiskLow(RuntimeError):
    """Места на диске впритык — сбор надо остановить, пока он не забил его совсем.

    Порог не фиксированный: публикация базы делает её ПОЛНУЮ копию (sqlite
    backup во временный файл), поэтому свободного места всегда должно хватать
    минимум на ещё одну базу. Иначе получим худший исход: диск забит, собранное
    не опубликовать, а SQLite рискует поймать обрыв записи.
    """

    def __init__(self, free_mb: float, need_mb: float):
        self.free_mb = round(free_mb)
        self.need_mb = round(need_mb)
        self.collected = 0
        super().__init__(f'Мало места: свободно {self.free_mb} МБ, нужно ≥ {self.need_mb} МБ')


# Минимум свободного места, МБ. 0 = проверка выключена (по умолчанию для CLI на
# своей машине, где диска вдоволь). Сервис на сервере выставляет своё значение.
MIN_FREE_MB = 0
_db_file = ''          # путь к базе; ставит run_continuous — для check_disk


def check_disk():
    """Бросает DiskLow, если места мало. Запас считаем от текущего веса базы:
    он растёт вместе с ней, поэтому порог не устаревает."""
    if MIN_FREE_MB <= 0 or not _db_file:
        return
    p = Path(_db_file)
    try:
        free = shutil.disk_usage(p.parent).free / 1e6
        db = p.stat().st_size / 1e6 if p.exists() else 0
    except OSError:
        return  # не смогли узнать — не мешаем сбору
    need = max(MIN_FREE_MB, db * 1.5)   # ×1.5: копия базы при публикации + запас
    if free < need:
        raise DiskLow(free, need)
...
class RateLimiter:
    ...
    Riot считает лимит на пару (ключ, хост), поэтому у каждого хоста свой
    лимитер: euw1/na1/kr и europe/americas/asia жмутся независимо и параллельно.
    Планирование через next_at (а не sleep под локом) — потоки, целящие в один
    хост, честно встают в очередь, но не блокируют потоки других хостов.
    """
...
def limiter_for(host: str) -> RateLimiter:
    """Лимитер хоста (euw1 / europe / …). Создаётся лениво, один на хост."""
    with _limiters_lock:
        lim = _limiters.get(host)
        if lim is None:
            lim = _limiters[host] = RateLimiter(RATE_DELAY)
        return lim


def api_call(fn, *args, **kwargs):
    # Первый аргумент любого вызова riotwatcher — хост роутинга (euw1/europe/…),
    # по нему и выбираем лимитер: платформенные и региональные лимиты раздельны,
    # а euw1+eun1 корректно делят один лимит `europe` на MATCH-V5.
    host = str(args[0]) if args else 'global'
    limiter = limiter_for(host)
    net_retries = 0
    while True:
        try:
            limiter.wait()
...
def collect_bucket(watcher, con, platform: str, regional: str, bucket: str,
    ...
    for puuid in iter_players(watcher, platform, bucket, seen):
        ...
        for mid in (match_ids or []):
            ...
            if not match:
                continue
            check_disk()   # предохранитель: не забиваем диск до отказа
            # Разбор+запись матча — под локом: пишем из нескольких потоков-регионов.
            with DB_LOCK:
...
def run_continuous(api_key: str, db_path: str, regions: list, buckets: list,
                   days: int, cap: int):
    import datetime
    global COMPLETED_ITEMS, _db_file
    ...
    def run_region(region: str) -> int:
        """Один регион: свои бакеты сверху вниз. Регионы идут параллельно —
        у каждого хоста свой лимит Riot, так что они не мешают друг другу."""
        nonlocal session_total
        regional = PLATFORM_TO_REGIONAL[region]
        got_region = 0
        for bucket in buckets:
            if stop.is_set():
                break
            print(f'\n==== [{region}] бакет {bucket} -> {regional} | '
                  f'в базе: {db_total(con)} ====', flush=True)
            with total_lock:
                so_far = session_total
            got = collect_bucket(watcher, con, region, regional, bucket,
                                 start_time, cap, so_far)
            with total_lock:
                session_total += got
                reached = cap and session_total >= cap
            got_region += got
            print(f'---- [{region}] {bucket}: +{got} | в базе: {db_total(con)} | '
                  f'за сессию: {session_total} ----', flush=True)
            if reached:
                stop.set()
                break
        return got_region
    ...
    try:
        # Поток на регион. Лимит Riot считается на пару (ключ, хост), поэтому
        # euw1/na1/kr жмутся одновременно и дают кратный прирост; регионы с общим
        # хостом MATCH-V5 (euw1+eun1 → europe) сами встанут в очередь в лимитере.
        ...
        # Диск на пределе: тормозим ДО того, как забьём его совсем — иначе не
        # останется места даже опубликовать собранное (публикация копирует базу).
        stop.set()
        expired = e
