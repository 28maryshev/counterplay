...
public sealed class RecommendationEngine : IDisposable
{
    ...
    // Тир-лист как у op.gg: грейд не по чистому винрейту, а по META-SCORE —
    // сила (нижняя граница Уилсона) + ПРИСУТСТВИЕ (пик- и бан-рейт). Иначе наверх
    // лезут низкопикрейтные однотрики с раздутым WR, а реальные мета-пики (их все
    // пикают/банят при ~49-51% WR) проваливаются. Веса откалиброваны по данным.
    ...
    // Грейд — по РАНГУ meta-score внутри роли (как тиры op.gg): доля champ'ов роли
    // выше данного. Ранговая шкала устойчива к масштабу meta (с банами или без) и
    // всегда даёт спред S..D. Пороги дают «пирамиду»: S ≈ верх 5%, дальше шире.
